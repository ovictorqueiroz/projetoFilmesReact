...
export default function App() {
  ...
  const [telaAtiva, setTelaAtiva] = useState('admin'); // 'admin' | 'perfil'
  ...
  // Admin vendo o próprio perfil
  if (isAdmin && telaAtiva === 'perfil') {
    return (
      <ScrollView style={styles.container}>
        <Text style={styles.titulo}>🎬 Meus Filmes 🍿</Text>
        <TouchableOpacity style={styles.botaoAdmin} onPress={() => setTelaAtiva('admin')}>
          <Text style={styles.botaoAdminTexto}>⚙️ Painel Admin</Text>
        </TouchableOpacity>
        <FilmeForm />
  ...
  // Admin vendo o painel
  if (isAdmin) {
    return <AdminPage onNavegar={setTelaAtiva} />;
  }
